...
def test_deadzone_ignores_noise():
    # cd_time bewusst gross gewaehlt, damit dieser reine Totzone-Test nicht
    # zufaellig in die (jetzt kuerzere) Keepalive-Schwelle laeuft
    cfg = PassiveControllerConfig(deadzone_w=40, min_setpoint_change_w=50, max_step_w=125,
                                   min_send_interval_s=0, default_cd_time_s=1000)
...
def test_keepalive_resends_unchanged_setpoint_before_cd_time_expires():
    ...
    echtes Update noetig), muss trotzdem nach der Haelfte von cd_time
    ...
    # t=25s: cd_time=60s, Keepalive-Schwelle bei der Haelfte = 30s ->
    # noch nicht erreicht -> weiterhin kein Senden
    assert ctrl.update(305, now=25.0) is None

    # t=31s: Schwelle (30s) ueberschritten -> Keepalive muss trotz Totzone senden
    cmd = ctrl.update(305, now=31.0)
...
def test_keepalive_does_not_trigger_before_half_cd_time():
    cfg = PassiveControllerConfig(deadzone_w=40, min_setpoint_change_w=50, max_step_w=125,
                                   min_send_interval_s=0, default_cd_time_s=100, max_cd_time_s=3600)
    ctrl = PassiveController(cfg)
    ctrl.update(300, now=0.0)
    # Haelfte von cd_time=100 -> Schwelle bei 50s
    assert ctrl.update(305, now=49.0) is None
    cmd = ctrl.update(305, now=51.0)
    assert cmd is not None
...
def test_keepalive_bypasses_holdoff():
    ...
    ctrl = PassiveController(cfg)
    ctrl.update(300, now=0.0)
    # Keepalive-Schwelle bei der Haelfte (30s) liegt VOR dem Hold-off-Ende bei 55s
    cmd = ctrl.update(305, now=35.0)
...
def test_discharge_cap_keepalive_still_works_while_capped():
    ...
    Keepalive nach der Haelfte von cd_time weiterhin senden."""
    cfg = PassiveControllerConfig(deadzone_w=5, min_setpoint_change_w=5, max_step_w=2000,
                                   min_output_w=-1500, max_output_w=800, min_send_interval_s=0,
    ...
    # Haelfte von cd_time (60/2=30s) ueberschritten -> Keepalive muss senden
