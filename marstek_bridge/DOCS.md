...
| `max_step_w` | 125 | Max. Leistungsschritt pro Zyklus, W (Obergrenze - siehe `step_gain` fuer die tatsaechliche Schrittberechnung) |
...
| `step_gain` | 1.0 | Proportionaler Faktor fuer den Schritt pro Zyklus: `Schritt = clamp(Abweichung * step_gain, -max_step_w, max_step_w)`. Standard 1.0 entspricht dem alten, festen Schrittbegrenzer (Abweichungen unterhalb `max_step_w` werden in einem Zyklus voll uebernommen). Niedrigere Werte (z.B. 0.3-0.5) daempfen zusaetzlich kleine Abweichungen fuer ein ruhigeres Verhalten, waehrend grosse Abweichungen weiterhin mit bis zu `max_step_w`/Zyklus konvergieren - kein Zielkonflikt mehr zwischen "schnell bei grossen Spruengen" und "ruhig bei kleinem Rauschen". |
| `zero_crossing_hysteresis_w` | 0.0 | Schwelle (W), die ein neuer Zielwert jenseits der Null ueberschreiten muss, damit ein Vorzeichenwechsel (Laden↔Entladen) zugelassen wird. Darunter wird der Zielwert auf 0W "eingefangen" statt die Richtung zu wechseln - verhindert haeufiges Umschalten zwischen Laden/Entladen bei Lasten, die knapp um den Nullpunkt pendeln. Standard 0 = deaktiviert (sofortiger Wechsel wie bisher). |
